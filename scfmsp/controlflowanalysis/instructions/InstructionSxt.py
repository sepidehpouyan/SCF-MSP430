...
class InstructionSxt(AbstractInstruction):
    name = 'sxt'
    ...
    def execute_judgment(self, ac):
        if(self.register_mode):
            domain = ac.ra.get(self.arguments[0]) & ac.secenv.get(self.get_execution_point())
            ac.ra.set(self.arguments[0], domain)
        
        else:
            domain = ac.mem & ac.secenv.get(self.get_execution_point())
            ac.mem = domain
